...
include_once '../../control/Admin/TbReport.php';
if (!isset($_SESSION['user_id'])) {
  header('Location: ../../index.php');
}
if (!empty($_POST['cedula'])) {
  $Consulta = AdminReport("filter", $_POST['cedula']);
} else {
  $Consulta = AdminReport("all", 0);
}
if (isset($_POST['id'])) {
  deleteReport($_POST['id']);
  $Consulta = AdminReport("all", 0);
  $messageSuccess = "Reporte eliminado";
}
?>
<!doctype html>
<html lang="en">
    ...
    <nav class="navbar-expand-lg navbar-light bg-light">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="AdminReport.php">Reportes</a>
          </li>
        </ul>
      </div>
    </nav>
      ...
      <div class="col-sm-12">
        <div class="row">
          &nbsp;&nbsp;&nbsp;&nbsp; <nav class="navbar-light bg-light">
            <form class="form-inline" action="AdminReport.php" method="post">
          ...
          <table class="table">
            ...
            <tbody>
              <?php
              while ($reportes = $Consulta->fetch(PDO::FETCH_ASSOC)) {
              ?>
                <tr>
                  <td><?php echo $reportes['id'] ?></td>
                  <td><?php echo $reportes['idU'] ?></td>
                  <td><?php echo $reportes['sintomas'] ?></td>
                  <td><?php echo $reportes['fecha'] ?></td>
                  <td>
                    <form action="EditReport.php" method="post">
                      <input type="hidden" name="id" value="<?php echo $reportes['id'] ?>">
                      <input type="hidden" name="sintomas" value="<?php echo $reportes['sintomas'] ?>">
                      <input type="hidden" name="fecha" value="<?php echo $reportes['fecha'] ?>">
                      <button class="btn btn-info"> <i class="fas fa-pen"></i></button>
                    </form>
                  </td>
                  <td>
                    <form action="AdminReport.php" method="post" id="miFormulario">
                      <input type="hidden" name="id" value="<?php echo $reportes['id'] ?>">
