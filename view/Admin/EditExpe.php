...
if (!empty($_POST['idE']) && !empty($_POST['exper'])) {
  $sql = "UPDATE experiencias  SET experiencia = :experiencia ,fecha = :fecha WHERE id = :id";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id', $_POST['idE']);
  $stmt->bindParam(':experiencia', $_POST['exper']);
  $stmt->bindParam(':fecha', $_POST['fecha']);
  $message = "";
  if ($stmt->execute()) {
    echo "<script>alert('Datos ACtualizados ✅'); location='./AdminExpe.php'</script>";
  } else {
  ...
  <div class="container cont">
    <div class="row">
      <div class="col-md-12 order-md-1"><br><br><br><br>
        <h4 class="mb-3">Editar Experiencia</h4>
        <hr>
        <br>
        <form class="needs-validation" action="EditExpe.php" method="POST" novalidate>
          <div class="mb-3">

            <input type="hidden" class="form-control" id="address" name="idE" placeholder="" value="<?php echo $_POST['id'] ?>" required>
          ...
          <div class="row">
            <div class="col-md-8 mb-3">
              ...
              <textarea class="form-control" id="firstName" name="exper" placeholder="" required><?php echo $_POST['experiencia'] ?></textarea>

            </div>
            <div class="col-md-8 mb-8">
              <label for="lastName">fecha</label>
              <input type="text" class="form-control" id="lastName" name="fecha" placeholder="" value="<?php echo $_POST['fecha'] ?>" required>
              <div class="invalid-feedback">
          ...
          <br><br>
          <a href="AdminExpe.php" class="btn btn-danger">Cancelar</a>
